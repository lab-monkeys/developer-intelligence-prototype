...
export const DeploymentFrequencyReport = ({ reportDeploymentFrequencyData, showReportDeploymentFrequencyData, setShowReportDeploymentFrequencyData }) => {

  function handleClick() {
    setShowReportDeploymentFrequencyData(false)
  }
  ...
  return (
    <>
      <div data-state={showReportDeploymentFrequencyData ? 'open' : 'closed'} role="dialog" className="fixed z-50 gap-4 bg-background p-8 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500 inset-y-0 top-8 right-8 h-[calc(100%-4rem)] w-3/4 rounded-2xl border-l data-[state=closed]:slide-out-to-right data-[state=closed]:opacity-0 data-[state=closed]:pointer-events-none data-[state=open]:slide-in-from-right sm:max-w-xl">
        ...
        {(reportDeploymentFrequencyData !== null && reportDeploymentFrequencyData?.activePayload[1]?.value < reportDeploymentFrequencyData?.activePayload[0]?.value[0] || reportDeploymentFrequencyData?.activePayload[1]?.value > reportDeploymentFrequencyData?.activePayload[0]?.value[1]) && (
          <AlertAnomaly />
        )}
        {(reportDeploymentFrequencyData !== null && reportDeploymentFrequencyData?.activePayload[1]?.value >= reportDeploymentFrequencyData?.activePayload[0]?.value[0] && reportDeploymentFrequencyData?.activePayload[1]?.value <= reportDeploymentFrequencyData?.activePayload[0]?.value[1]) && (
          <AlertHealthy />
      ...
      <div data-state={showReportDeploymentFrequencyData ? 'open' : 'closed'} className="fixed inset-0 z-40 bg-background/35 backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=open]:opacity-1 data-[state=closed]:opacity-0 data-[state=closed]:pointer-events-none" onClick={handleClick} />
    </>
  )
}