...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const reqUrl = `${process.env.PELORUS_API_URL}/sdp/lead_time_for_change/${appName}/data?range=${getDaysBetweenDates({ to: toDate, from: fromDate })}d&start=${Math.floor(toDate.getTime() / 1000)}`;
